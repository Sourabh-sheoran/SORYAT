    ...
    <div className="relative z-10 w-full pt-28 pb-20 px-5 sm:px-8 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-normal text-neutral-950 dark:text-white leading-tight tracking-tight">
            Proof of Work. <br />
        ...
        <p className="text-sm sm:text-base text-neutral-700 dark:text-neutral-300 max-w-md">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-mono transition-all cursor-pointer ${
              activeCategory === cat
                ...
                : "bg-neutral-100 dark:bg-white/5 text-neutral-700 dark:text-neutral-300 border border-neutral-200 dark:border-white/10 hover:bg-neutral-200 dark:hover:bg-white/10"
            }`}
          >
            {cat}
          </button>
          ...
          >
            ...
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between text-[10px] font-mono text-neutral-600 dark:text-neutral-400 font-medium">
                ...
                <p className="text-xs text-neutral-700 dark:text-neutral-300 leading-relaxed line-clamp-2 font-normal">
                  {proj.tagline}
                </p>
                ...
                <div className="flex flex-wrap gap-1 pt-2">
                  {proj.techStack.slice(0, 3).map((t) => (
                    <span key={t} className="text-[9px] font-mono px-2 py-0.5 rounded-md bg-neutral-200/80 dark:bg-white/10 text-neutral-800 dark:text-neutral-200 border border-neutral-300 dark:border-white/10 font-medium">
                      {t}
                    </span>
                  ))}
                  {proj.techStack.length > 3 && (
                    <span className="text-[9px] font-mono px-2 py-0.5 rounded-md bg-neutral-100 dark:bg-white/5 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-white/5">
                      +{proj.techStack.length - 3}
                    </span>
                  )}
                </div>
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          <div
            ...
            className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in"
          />

          <div className="relative w-full max-w-3xl bg-white dark:bg-[#111111] border border-neutral-200 dark:border-white/15 rounded-3xl p-6 sm:p-8 shadow-2xl z-10 my-8 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto text-neutral-900 dark:text-white">
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute top-5 right-5 p-2 rounded-full bg-neutral-100 dark:bg-white/10 hover:bg-neutral-200 dark:hover:bg-white/20 text-neutral-700 dark:text-neutral-300 hover:text-black dark:hover:text-white transition-colors cursor-pointer"
            >
              <X size={20} />
            </button>

            <div className="mb-6">
              <div className="flex items-center gap-2 font-mono text-xs text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-1 font-semibold">
            ...
            {/* Metrics Row */}
            <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-neutral-50 dark:bg-white/5 border border-neutral-200 dark:border-white/10 mb-6 text-center">
              {selectedProject.metrics.map((m) => (
                <div key={m.label}>
                  <div className="font-mono text-xl sm:text-2xl font-bold text-neutral-950 dark:text-white">
                    {m.value}
                  </div>
                  <div className="text-[10px] font-mono text-neutral-600 dark:text-neutral-400 uppercase mt-0.5 font-semibold">
                    {m.label}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-4 text-xs sm:text-sm leading-relaxed mb-6">
              <div className="p-4 rounded-2xl bg-red-500/5 dark:bg-red-500/10 border border-red-500/20 text-neutral-800 dark:text-neutral-200">
                <strong className="text-red-600 dark:text-red-400 font-mono text-xs uppercase block mb-1">
                  The Challenge
                </strong>
                <p>{selectedProject.challenge}</p>
              </div>

              <div className="p-4 rounded-2xl bg-blue-500/5 dark:bg-blue-500/10 border border-blue-500/20 text-neutral-800 dark:text-neutral-200">
                <strong className="text-blue-600 dark:text-blue-400 font-mono text-xs uppercase block mb-1">
                  Our Engineering Solution
                </strong>
                <p>{selectedProject.solution}</p>
              </div>

              <div className="p-4 rounded-2xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 text-neutral-800 dark:text-neutral-200">
                <strong className="text-emerald-600 dark:text-emerald-400 font-mono text-xs uppercase block mb-1">
                  The Result & ROI
                </strong>
            ...
            {/* Tech Stack */}
            <div className="mb-8">
              <div className="text-xs font-mono uppercase tracking-wider text-neutral-600 dark:text-neutral-400 mb-2 font-bold">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedProject.techStack.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 rounded-lg bg-neutral-100 dark:bg-white/10 border border-neutral-200 dark:border-white/15 text-xs font-mono text-neutral-800 dark:text-neutral-200 font-medium"
                  >
                    {tech}
                  </span>
                ))}
            ...
            {/* Action */}
            <div className="pt-4 border-t border-neutral-200 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-full bg-neutral-900 dark:bg-white text-white dark:text-neutral-950 font-bold text-xs hover:bg-black dark:hover:bg-neutral-100 transition-all flex items-center justify-center gap-1.5 shadow-md cursor-pointer"
              >
